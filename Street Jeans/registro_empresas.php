...
if (isset($_POST['acao'])) {
    ...
    $sql = $pdo->prepare("INSERT INTO `empresa` values(null,?,?,?,?,?,?,?,?,?)");

    $sql->execute(array($nome, $cnpj, $cep, $bairro, $endereco, $complemento, $email, $telefone, $senhapin));
...
for ($i=0; $i < count($info); $i++) {
  $codigo = $info[$i]['id_empresa'];
}

if (isset($_POST['acao'])) {
  for ($i=0; $i < count($info); $i++) {
    $codigoempresa = $info[$i]['id_empresa'];
  }
  print"<script>alert('Seu ID é $codigoempresa e poderá ser usado para apagar seu registro, certifique-se de que irá lembrar deste código')</script>";
}
...
<div class="perguntas">
    <ol>
      <form class="" action="registro_empresas.php" method="post">


        <li><label for="nome">Nome:</label>
         <input type="text" name="nome" required></li>
         <br>
         <li><label for="cnpj">CNPJ:</label>
          <input type="text" name="cnpj" required></li>
          <br>
        ...
        <input type="text" name="complemento"></li>
        <br>
